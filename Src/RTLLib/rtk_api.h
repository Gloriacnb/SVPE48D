...
typedef struct
{
    uint16 value[RTK_DOT_1AS_TIMESTAMP_UNIT_IN_WORD_LENGTH];
...
typedef uint8  rtk_dscp_t;         /* dscp vlaue */
...
typedef uint16  rtk_fid_t;        /* filter id type */
...
typedef struct
{
    ...
    /* CVLAN acton */
    uint16        filterIngressCvlanIdx;
    uint16        filterIngressCvlanVid;

    /* SVLAN action */
    uint16        filterEgressSvlanIdx;

    /* Policing action */
    uint16        filterPolicingIdx[FILTER_POLICING_MAX];

    /* Forwarding action */
    uint16        filterRedirectPortmask;
    uint16        filterAddDstPortmask;
    
    /* QOS action */
    uint16        filterPriority;
} rtk_filter_action_t;
...
typedef uint16 rtk_filter_op_t;
...
typedef struct
{
    uint16 addr[RTK_IPV6_ADDR_WORD_LENGTH];
...
typedef uint16 rtk_filter_number_t;

typedef struct
{
    uint16 value[FILTER_PATTERN_MAX];
    uint16 mask[FILTER_PATTERN_MAX];
...
typedef uint16 rtk_filter_field_raw_t;
...
typedef uint16  rtk_filter_id_t;    /* filter id type */
...
typedef uint16 rtk_filter_port_t;

typedef uint16 rtk_filter_state_t;

typedef uint16 rtk_filter_unmatch_action_t;
...
typedef uint16 rtk_l2_flushItem_t;
...
typedef uint8  rtk_mac_cnt_t;     /* meter id type  */
...
typedef uint16  rtk_meter_id_t;     /* meter id type  */

typedef uint8  rtk_mode_t;

typedef uint8  rtk_port_t;        /* port is type */
...
{   
	...
	uint8    Half_10;          /*PHY register 4.5 setting for 10BASE-TX half duplex capable*/
	uint8    Full_10;          /*PHY register 4.6 setting for 10BASE-TX full duplex capable*/
	uint8    Half_100;         /*PHY register 4.7 setting for 100BASE-TX half duplex capable*/
	uint8    Full_100;         /*PHY register 4.8 setting for 100BASE-TX full duplex capable*/
	uint8    Full_1000;        /*PHY register 9.9 setting for 1000BASE-T full duplex capable*/
...
typedef uint16  rtk_port_phy_data_t;     /* phy page  */

//typedef uint16  rtk_port_phy_page_t;     /* phy page  */
...
{
    uint8  bits[RTK_TOTAL_NUM_OF_WORD_FOR_1BIT_PORT_LIST];
} rtk_portmask_t;

typedef uint8  rtk_pri_t;         /*priority value */
typedef uint8  rtk_dei_t;        /*dei value*/
...
typedef uint8  rtk_qid_t;        /* queue id type */

typedef struct rtk_qos_pri2queue_s
{
    uint8 pri2queue[RTK_MAX_NUM_OF_PRIORITY];
} rtk_qos_pri2queue_t;

typedef struct rtk_qos_queue_weights_s
{
    uint16 weights[RTK_MAX_NUM_OF_QUEUE];
...
typedef uint8  rtk_queue_num_t;    /* queue number*/
...
typedef uint16  rtk_rate_t;     /* rate type  */
...
{
    uint64 ifInOctets;
    uint16 dot3StatsFCSErrors;
    uint16 dot3StatsSymbolErrors;
    uint16 dot3InPauseFrames;
    uint16 dot3ControlInUnknownOpcodes;
    uint16 etherStatsFragments;
    uint16 etherStatsJabbers;
    uint16 ifInUcastPkts;
    uint16 etherStatsDropEvents;
    uint64 etherStatsOctets;
    uint16 etherStatsUndersizePkts;
    uint16 etherStatsOversizePkts;
    uint16 etherStatsPkts64Octets;
    uint16 etherStatsPkts65to127Octets;
    uint16 etherStatsPkts128to255Octets;
    uint16 etherStatsPkts256to511Octets;
    uint16 etherStatsPkts512to1023Octets;
    uint16 etherStatsPkts1024toMaxOctets;
    uint16 etherStatsMcastPkts;
    uint16 etherStatsBcastPkts;
    uint64 ifOutOctets;
    uint16 dot3StatsSingleCollisionFrames;
    uint16 dot3StatsMultipleCollisionFrames;
    uint16 dot3StatsDeferredTransmissions;
    uint16 dot3StatsLateCollisions;
    uint16 etherStatsCollisions;
    uint16 dot3StatsExcessiveCollisions;
    uint16 dot3OutPauseFrames;
    uint16 dot1dBasePortDelayExceededDiscards;
    uint16 dot1dTpPortInDiscards;
    uint16 ifOutUcastPkts;
    uint16 ifOutMulticastPkts;
    uint16 ifOutBrocastPkts;
    uint16 outOampduPkts;
    uint16 inOampduPkts;
    uint16 pktgenPkts;
...
typedef uint16  rtk_stg_t;        /* spanning tree instance id type */
...
typedef uint16  rtk_stp_msti_id_t;     /* MSTI ID  */
...
typedef uint16 rtk_svlan_index_t;
...
typedef uint16 rtk_svlan_tpid_t;
...
typedef uint16  rtk_vlan_proto_type_t;     /* protocol and port based VLAN protocol type  */
...
typedef uint16  rtk_vlan_t;        /* vlan id type */
...
{
    uint8 phyport;   /*the physical port where packet received*/
    uint8 protocol;  /*Acl protocol which is in the packet*/
    uint16 datavalue;       /*16bit ether type or TCP/UDP source port, destination port value*/
    uint8 priority;   /*Acl priority assigned to the packet*/
    uint8 action;     /*How to deal with the packet*/
} rtk_filter_rule_t;
