...
* 
* $Revision: 11247 $ 
* $Date: 2010-07-22 09:58:37 +0800 (星期四, 22 七月 2010) $
*
...
#include "../RTLLib/rtl8306e_asicdrv.h"

#include "../RTLLib/rtl8306e_types.h"
#ifndef RTK_X86_ASICDRV
#include "../RTLLib/mdcmdio.h"
#endif
...
int8 rtl8306e_reg_set(uint8 phyad, uint8 regad, uint8 npage, uint16 value)
{
...
int8 rtl8306e_reg_get(uint8 phyad, uint8 regad, uint8 npage, uint16 *pvalue)
{
...
int8 rtl8306e_regbit_set(uint8 phyad, uint8 regad, uint8 pos, uint8 npage,  uint8 value)
{
    uint16 rdata;
    ...
    if (value) 
        rtl8306e_reg_set(phyad, regad, npage, rdata | (1 << pos));
    else
        rtl8306e_reg_set(phyad, regad, npage, rdata & (~(1 << pos)));
    return SUCCESS;
}
...
int8 rtl8306e_regbit_get(uint8 phyad, uint8 regad, uint8 pos, uint8 npage,  uint8 * pvalue)
{
    uint16 rdata;
...
int8 rtl8306e_phyReg_set(uint8 phyad, uint8 regad, uint8 npage, uint16 value)
{
...
int8 rtl8306e_phyReg_get(uint8 phyad, uint8 regad, uint8 npage, uint16 *pvalue)
{
...
int8 rtl8306e_asic_checkOUI(void) {
	uint16 regval;
	rtl8306e_reg_get(0, 2, 0, &regval);
	if( regval == 0x001c ) {
		return SUCCESS;
	}
	return FAILED;
}
int8 rtl8306e_asic_init(void)
{
    uint16 regval;
    uint16 regval2;
    uint16 regval3;
 ...
 * Note:
 */
int8 rtl8306e_phy_reset(uint8 phy)
{
...
int8 rtl8306e_switch_maxPktLen_set(uint8 maxLen)
{
    switch(maxLen)
    {
 ...
 *              
 */ 
int8 rtl8306e_switch_maxPktLen_get(uint8 *pMaxLen)
{
    uint8 regval;
 ...
 */
int8 rtl8306e_port_etherPhy_set(uint8 phy, uint8 autoNegotiation, uint8 advCapability, uint8 speed, uint8 fullDuplex)
{
    uint16 ctrlReg;
...
int8 rtl8306e_port_etherPhy_get(uint8 phy, uint8 *pAutoNegotiation, uint8 *pAdvCapability, uint8 *pSpeed, uint8 *pFullDuplex)
{
    uint16 regData;
 ...
 */
int8 rtl8306e_port_port5LinkStatus_set(uint8 enabled)
{
    uint8 duplex, speed, nway;
...
int8 rtl8306e_port_port5LinkStatus_get(uint8 *pEnabled)
{
    uint8 bitValue;
...
int8 rtl8306e_port_phyLinkStatus_get(uint8 phy, uint8 *plinkUp)
{
    uint8 bitValue;
...
int8 rtl8306e_port_phyAutoNegotiationDone_get(uint8 phy, uint8 *pDone)
{
    uint8 bitValue;
...
int8 rtl8306e_port_phyLoopback_set(uint8 phy, uint8 enabled)
{
    if(phy > 4)
        return FAILED;
...
int8 rtl8306e_port_phyLoopback_get(uint8 phy, uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 *
 */
int8 rtl8306e_portLearningAbility_set(uint8 port, uint8 enabled)
{
...
int8 rtl8306e_port_isolation_set(uint16 isomsk)
{
    uint16 regval;
...
int8 rtl8306e_port_isolation_get(uint16 *pIsomsk)
{
    uint16 regval;
 ...
 */
int8 rtl8306e_vlan_tagAware_set(uint8 enabled)
{
...
int8 rtl8306e_vlan_tagAware_get(uint8 * pEnabled)
{
    uint8 bitValue;
 ...
 *
 */
int8 rtl8306e_vlan_IgrFilterEnable_set(uint8 enabled)
{
 ...
 *
 */
int8 rtl8306e_vlan_IgrFilterEnable_get(uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 */
int8 rtl8306e_vlan_leaky_set(uint8 type, uint8 enabled)
{
    switch(type) 
    {
...
int8 rtl8306e_vlan_leaky_get(uint8 type, uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 */
int8 rtl8306e_vlan_nullVidReplace_set(uint8 port, uint8 enabled)
{
    uint8 speed, duplex, nway;
 ...
 *      frame depends on the configuration.
 */
int8 rtl8306e_vlan_nullVidReplace_get(uint8 port, uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 */
int8 rtl8306e_vlan_portPvidIndex_set(uint8 port, uint8 vlanIndex)
{
    uint16 regValue;
...
int8 rtl8306e_vlan_portPvidIndex_get(uint8 port, uint16 *pVlanIndex)
{
 ...
 */
int8 rtl8306e_vlan_portAcceptFrameType_set(uint8 port, rtl8306e_acceptFrameType_t accept_frame_type)
{
    uint16 regval;
 ...
 *    RTL8306E_ACCEPT_UNTAG_ONLY
 */
int8 rtl8306e_vlan_portAcceptFrameType_get(uint8 port, rtl8306e_acceptFrameType_t *pAccept_frame_type)
{
    uint16 regval;
 ...
 */
int8 rtl8306e_vlan_tagInsert_set(uint8 egPort, uint8 igPortMsk)
{
    uint16 regval;
 ...
 *      is in the igPortMsk, it will be inserted with an VLAN tag.
 */
int8 rtl8306e_vlan_tagInsert_get(uint8 egPort, uint8 * pIgPortMsk)
{
    uint16 regval ;
    uint8 bitval;
    ...
    rtl8306e_regbit_get(0, 28, 0, 3, &bitval);
    if(!bitval)
        *pIgPortMsk = 0;
 ...
 */
int8 rtl8306e_vlan_entry_set(uint8 vlanIndex, uint16 vid, uint8 mbrmsk, uint8 untagmsk )
{
    uint16 regValue;
 ...
 *     For example, mbrmask 23=0x17=010111 means port 0,1,2,4 in the member set.
 */
int8 rtl8306e_vlan_entry_get(uint8 vlanIndex, uint16 *pVid, uint16 *pMbrmsk, uint16 *pUntagmsk)
{
 ...
 *       
 */
int8 rtl8306e_vlan_vlanBasedPriority_set(uint8 vlanIndex, uint8 pri)
{
    uint16 regval;
 ...
 *       
 */
int8 rtl8306e_vlan_vlanBasedPriority_get(uint8 vlanIndex, uint8 *pPri)
{
    uint16 regval;
 ...
 */
int8 rtl8306e_vlan_transEnable_set(uint8 enable, uint8 portmask)
{
    uint16 regval;
 ...
 *      at the same time, user should choose one of them. 
 */
int8 rtl8306e_vlan_transEnable_get(uint8 *pEnable, uint8 *pPortMask)
{
    uint16 regval;
 ...
 * Note:
 */
int8 rtl8306e_vlan_transparentEnable_set(uint8 enable)
{
    uint8 port;
 ...
 *      new VID.
 */ 
int8 rtl8306e_vlan_transVid_set(uint8 vlanIndex, uint16 transVid)
{
   uint16 regval;
 ...
 *      new VID.
 */ 
int8 rtl8306e_vlan_transVid_get(uint8 vlanIndex, uint16 *pTransVid)
{
    uint16 regval;
 ...
 *       
 */
int8 rtl8306e_svlan_tagAdmit_set(uint8 port, uint8 enable)
{
    if (port > 5)
        return FAILED;
 ...
 *       
 */
int8 rtl8306e_svlan_tagAdmit_get(uint8 port, uint8 *pEnable)
{
    uint16 regval;
 ...
 */
int8 rtl8306e_svlan_otagSrc_set(uint8 port, uint16 ovidSrc, uint16 opriSrc)
{
    uint16 regval;
...
int8 rtl8306e_svlan_otagSrc_get(uint8 port, uint16 *pOvidsrc, uint16 *pOpriSrc)
{
    uint16 regval;
 ...
 */
int8 rtl8306e_cpu_set(uint8 port, uint8 enTag)
{
    uint16 regValue;
 ...
 *      that no port is assigned as cpu port
 */
int8 rtl8306e_cpu_get(uint8 *pPort, uint8 *pEnTag)
{
    uint16 regValue;
    uint8 bitValue, bitVal, cpufun;
...
int8 rtl8306e_qos_softReset_set(void)
{    
    /*software reset*/
...
int8 rtl8306e_qos_queueNum_set(uint8 num)
{
    uint16 regValue;
 ...
 *      Queue number is global configuration for switch.
 */
int8 rtl8306e_qos_queueNum_get(uint8 *pNum)
{
    uint16 regValue;
 ...
 */
int8 rtl8306e_qos_priToQueMap_set(uint8 priority, uint8 qid)
{
    uint16 regValue;
 ...
 *      we can use this function to set pkt priority with queue id mapping
 */
int8 rtl8306e_qos_priToQueMap_get(uint8 priority, uint8 *pQid)
{
    uint16 regValue;
 ...
 */
int8 rtl8306e_qos_portRate_set(uint8 port, uint16 n64Kbps, uint8 direction, uint8 enabled)
{
    uint16 regValue;
 ...
 *      is configured per port, but for input rate control, it is for all port.
 */
int8 rtl8306e_qos_portRate_get(uint8 port, uint16 *pN64Kbps, uint8 direction, uint8 *pEnabled)
{
    uint16 regValue;
    uint8 bitValue;
    ...
    {
        /*Get port Rx rate*/
        rtl8306e_regbit_get(0, 21, 15, 3, &bitValue);
        *pEnabled = (bitValue ? FALSE:TRUE);
        rtl8306e_reg_get(port, 21, 2, &regValue);
 ...
 *      
 */
int8 rtl8306e_qos_1pRemarkEnable_set(uint8 port, uint8 enabled)
{
 ...
 *      
 */
int8 rtl8306e_qos_1pRemarkEnable_get(uint8 port, uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 *      cpu tag.
 */
int8 rtl8306e_qos_1pRemark_set(uint8 priority, uint8 priority1p)
{
    uint16 regValue;
 ...
 *      cpu tag.
 */
int8 rtl8306e_qos_1pRemark_get(uint8 priority, uint8 *pPriority1p)
{
    uint16 regValue;
 ...
 */
int8 rtl8306e_qos_portPri_set(uint8 port, uint8 priority)
{
    uint16 regValue;
 ...
 *      packet will be assigned a port-based priority correspond to the ingress port.
 */ 
int8 rtl8306e_qos_portPri_get(uint8 port, uint8 *pPriority)
{
    uint16 regValue;
 ...
 *      to 2-bit priority
 */ 
int8 rtl8306e_qos_1pPriRemap_set(uint8 tagprio, uint8 prio)
{
    uint16 regValue;
 ...
 *      to 2-bit priority
 */ 
int8 rtl8306e_qos_1pPriRemap_get(uint8 tagprio, uint8 *pPrio)
{
    uint16 regValue;
 ...
 *     
 */ 
int8 rtl8306e_qos_dscpPriRemap_set(uint8 dscpcode, uint8 priority)
{
    uint16 regValue1, regValue2;

    if ((dscpcode > RTL8306_DSCP_BF) ||(priority > RTL8306_PRIO3))
        return FAILED;
 ...
 *     
 */ 
int8 rtl8306e_qos_dscpPriRemap_get(uint8 dscpcode, uint8 *pPriority)
{
    uint16  regValue1, regValue2;

    if ((dscpcode > RTL8306_DSCP_BF) || (NULL == pPriority))
        return FAILED;
...
int8 rtl8306e_qos_priSrcArbit_set(rtl8306e_qosPriArbitPara_t priArbit)
{
    uint16 regval;
 ...
 *      will not be recognized any more. 
 */
int8 rtl8306e_qos_priSrcArbit_get(rtl8306e_qosPriArbitPara_t *pPriArbit)
{
    uint16 regval;
    uint8 bitval;
    uint8 i ;
    ...
    /*acl based priority*/
    rtl8306e_regbit_get(0, 28, 7, 3, &bitval);
    if(bitval)
    {
    ...
    /*dscp based priority*/
    rtl8306e_regbit_get(0, 26, 15, 3, &bitval);
    if (bitval)
    {
    ...
    /*1Q based priority*/
    rtl8306e_regbit_get(0, 26, 14, 3, &bitval);
    if (bitval)
    {
    ...
    /*port based priority*/
    rtl8306e_regbit_get(0, 26, 13, 3, &bitval);
    if (bitval)
    {
 ...
 */
int8 rtl8306e_qos_priSrcEnable_set(uint8 port, uint8 priSrc, uint8 enabled)
{
    uint8 duplex, speed, nway;
 ...
 *          RTL8306_CPUTAG_PRIO  - cpu tag priority
 */
int8 rtl8306e_qos_priSrcEnable_get(uint8 port, uint8 priSrc, uint8 *pEnabled)
{
    uint8 bitValue;
 ...
 * Note:
 */
int8 rtl8306e_qos_ipAddrPri_set(uint8 priority)
{
    uint16 regValue;
 ...
 * Note:
 */
int8 rtl8306e_qos_ipAddrPri_get(uint8 *priority)
{
    uint16 regValue;
 ...
 */
int8 rtl8306e_qos_ipAddr_set(uint8 entry, uint32 ip, uint32 mask, uint8 enabled)
{
    uint16 regValue;
 ...
 * Note:
 */
int8 rtl8306e_qos_ipAddr_get(uint8 entry, uint32 *pIp, uint32 *pMask , uint8 *pEnabled)
{
    uint16 hi, lo;
    uint8 bitValue;
 ...
 */
int8 rtl8306e_qos_schedulingPara_set(uint8 set, rtl8306e_qosSchPara_t sch_para)
{
    uint16 regValue;
 ...
 *      and q3_n64Kbps means Q2/Q3 queue bandwidth control, unit is 64Kbps.
 */
int8 rtl8306e_qos_schedulingPara_get(uint8 set, rtl8306e_qosSchPara_t *pSch_para)
{
    uint16 regValue;
 ...
 */
int8 rtl8306e_qos_portSchedulingMode_set(uint8 port, uint8 set, uint8 lbmsk)
{
    uint16 regValue;
 ...
 *      queue 3 and set 1 to enable it.
 */
int8 rtl8306e_qos_portSchedulingMode_get(uint8 port, uint8 *pSet, uint8 *pLbmsk)
{
    uint16 regValue;
...
int8 rtl8306e_qos_queFlcThr_set(uint8 queue, uint8 type, uint8 onoff, uint8 set, uint8 value)
{
    uint16 regValue, mask;
    uint8 selection;
    uint16 reg, shift;
...
int8 rtl8306e_qos_queFlcThr_get(uint8 queue, uint8 type, uint8 onoff, uint8 set, uint8* pValue)
{
    uint16 regValue, mask;
    uint8 selection, bitValue;
    uint16 reg, shift = 0;
    ...
    *pValue = 0;
    switch (selection) 
    {
        ...
        case 2 :    /*set 0, turn on, DSC*/
            ...
            *pValue = regValue & 0x3F;
            ...
            if (RTL8306_QUEUE0 == queue) 
            {
                rtl8306e_regbit_get(5, 21, 7, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else if (RTL8306_QUEUE1 == queue) 
            {
                rtl8306e_regbit_get(5, 21, 15, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else if (RTL8306_QUEUE2 == queue)
            {
                rtl8306e_regbit_get(5, 24, 7, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else  
            {
                rtl8306e_regbit_get(5, 24, 15, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }              
            break;
    ...
    case 3:   /*set 0, turn  on, QLEN*/
        ...
        if (RTL8306_QUEUE0 == queue) 
        {
            rtl8306e_regbit_get(5, 21, 6, 2, &bitValue);
            *pValue |= (bitValue << 6);
        }      
        else if (RTL8306_QUEUE1 == queue) 
        {
            rtl8306e_regbit_get(5, 21, 14, 2, &bitValue);
            *pValue |= (bitValue << 6);
        }      
        else if (RTL8306_QUEUE2 == queue)
        {
            rtl8306e_regbit_get(5, 24, 6, 2, &bitValue);
            *pValue |= (bitValue << 6);
        }      
        else  
        {
            rtl8306e_regbit_get(5, 24, 14, 2, &bitValue);
            *pValue |= (bitValue << 6);
        }               
        break;
        ...
        case 4:     /*set 1, turn off, DSC*/
            ...
            *pValue = (regValue & mask) >> shift;
            ...
            else if (RTL8306_QUEUE1 == queue) 
            {
                ...
                *pValue |= (regValue << 4);
                rtl8306e_regbit_get(1, 28, 0, 3, &bitValue);
                *pValue |= (bitValue << 6);
            }
            else if (RTL8306_QUEUE2 == queue) 
            {
        ...
        *pValue = (regValue & mask) >> shift;
        ...
        case 6:     /*set 1, turn on, DSC*/
            ...
            *pValue = (regValue & 0x3F) >> shift;
            ...
            if (RTL8306_QUEUE0 == queue) 
            {
                rtl8306e_regbit_get(5, 25, 7, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else if (RTL8306_QUEUE1 == queue) 
            {
                 rtl8306e_regbit_get(5, 25, 15, 2, &bitValue);
                 *pValue |= (bitValue << 6);
             }      
            else if (RTL8306_QUEUE2 == queue)
            {
                rtl8306e_regbit_get(5, 27, 7, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else  
            {
                rtl8306e_regbit_get(5, 27, 15, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
        ...
        case 7:      /*set 1, turn  on, QLEN*/
            ...
            if (RTL8306_QUEUE0 == queue) 
            {
                rtl8306e_regbit_get(5, 25, 6, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else if (RTL8306_QUEUE1 == queue) 
            {
                 rtl8306e_regbit_get(5, 25, 14, 2, &bitValue);
                 *pValue |= (bitValue << 6);
            }      
            else if (RTL8306_QUEUE2 == queue)
            {
                rtl8306e_regbit_get(5, 27, 6, 2, &bitValue);
                *pValue |= (bitValue << 6);
            }      
            else  
            {
                rtl8306e_regbit_get(5, 27, 14, 2, &bitValue);
                 *pValue |= (bitValue << 6);
            }               
            break;
...
int8 rtl8306e_qos_portFlcThr_set(uint8 port, uint8 onthr, uint8 offthr, uint8 direction )
{
    uint16 regValue;
...
int8 rtl8306e_qos_portFlcThr_get(uint8 port, uint8 *pOnthr, uint8 *pOffthr, uint8 direction)
{
    uint16 regValue;
...
int8 rtl8306e_qos_portFlcMode_set(uint8 port, uint8 set)
{
...
int8 rtl8306e_qos_portFlcMode_get(uint8 port , uint8 *set)
{
...
int8 rtl8306e_qos_queFlcEnable_set( uint8 port, uint8 queue, uint8 enabled)
{
...
int8 rtl8306e_qos_queFlcEnable_get(uint8 port, uint8 queue, uint8 *pEnabled)
{
    uint8 bitValue;
...
int8 rtl8306e_acl_entry_set(uint8 entryadd, uint8 phyport, uint8 action, uint8 protocol, uint16 datav, uint8 priority)
{
    ...
    rtl8306_TblBak.aclTbl[entryadd].data = datav;
 ...
 *       
 */
int8 rtl8306e_acl_entry_get(uint8 entryadd, uint8 *pPhyport, uint8 *pAction, uint8 *pProtocol, uint16  *pData, uint8 *pPriority)
{
    uint16 regValue;
    uint8 pollcnt  ;
    uint8 bitValue;
 ...
 */
int8 rtl8306e_mib_get(uint8 port, uint8 counter, uint32 *pValue)
{
    uint16 regValue1, regValue2;
 ...
 */
int8 rtl8306e_mibUnit_set(uint8 port, uint8 counter, uint8 unit)
{
 ...
 *      the other counters' counting uint is RTL8306_MIB_PKT
 */
int8 rtl8306e_mibUnit_get(uint8 port, uint8 counter, uint8 *pUnit)
{ 
    uint8 bitValue;
 ...
 * Note:
 */
int8 rtl8306e_mib_reset(uint8 port)
{
 ...
 */
int8 rtl8306e_mirror_portBased_set(uint8 mirport, uint8 rxmbr, uint8 txmbr)
{
    uint16 regValue;
 ...
 *      of port in txmbr will be mirrored to mirport.
 */
int8 rtl8306e_mirror_portBased_get(uint8 *pMirport, uint8 *pRxmbr, uint8* pTxmbr)
{
    uint16 regValue;
 ...
 *
 */
int8 rtl8306e_mirror_macBased_set(uint8 *macAddr, uint8 enabled)
{
 ...
 * 
 */
int8 rtl8306e_mirror_macBased_get(uint8 *macAddr, uint8 *pEnabled)
{
    uint16 regValue;
    uint8 bitValue;
 ...
 */
int8 rtl8306e_l2_MacToIdx_get(uint8 *macAddr, uint16* pIndex)
{
    uint8 tmp_index;
    uint8 status;
 ...
 *   
 */ 
static int8 rtl8306e_l2_IdxToMac_get(uint8 *macAddr, uint16* pIndex)
{
    uint16 tmp_index, regValue;
    uint8 status;
...
int8 rtl8306e_l2_unicastEntry_set(uint8 *macAddress, uint8 entry, uint8 age, uint8 isStatic, uint8 isAuth, uint8 port)
{
    uint16 regValue, index, pollcnt;
    uint8 bitValue;
...
int8 rtl8306e_l2_unicastEntry_get(uint8 *macAddress, uint16 entryAddr, uint8 *pAge, uint8 *pIsStatic, uint8 *pIsAuth, uint8 *pPort)
{
    uint16 entryAddrHd;
    uint16 regValue, pollcnt;
    uint8 bitValue;
...
int8 rtl8306e_fastGetAsicLUTUnicastEntry(uint8 *macAddress, uint8 entryAddr, uint8 *pAge, uint8 *pIsStatic, uint8 *pIsAuth, uint8 *pPort)
{    
    if ((NULL == macAddress) || (entryAddr > 0x7FF) || (NULL == pAge) || (NULL == pIsStatic ) ||
...
int8 rtl8306e_fastGetAsicLUTMulticastEntry(uint8 *macAddress, uint8 entryAddr, uint8 *pIsAuth, uint8 *pPortMask)
{
...
int8 rtl8306e_l2_multicastEntry_set(uint8 *macAddress, uint8 entry, uint8 isAuth, uint8 portMask)
{
    uint16 regValue, index, pollcnt;
    uint8 bitValue;
...
int8 rtl8306e_l2_multicastEntry_get(uint8 *macAddress, uint16 entryAddr, uint8 *pIsAuth, uint8 *pPortMask)
{
    uint16 entryAddrHd;
    uint16 regValue, pollcnt;
    uint8 bitValue;
...
int8 rtl8306e_l2_unicastMac_add(uint8 *macAddress, uint8 age, uint8 isStatic, uint8 isAuth, uint8 port, uint8 *pEntryaddr)
{
    int16 i;
    uint8 macAddr[6];
    uint16 index,entryaddr;
    uint8 isStatic1,isAuth1,age1,port1;
    ...
    for (i = 3; i >= 0; i--) 
    {
        ...
        {
            rtl8306e_l2_unicastEntry_set(macAddress, (uint16)i , age, isStatic, isAuth, port);
 ...
 */ 
int8  rtl8306e_l2_multicastMac_add(uint8 *macAddr,uint8 isAuth, uint8 portMask, uint8 *pEntryaddr)
{
    int8  i;
    uint8 macAddress[4][6];
    uint16 index;
    uint8 entryAddress[4], age[4];
    uint8 isStatic[4], isAuthority[4];
    uint8 port[4];
    uint8 isFull = TRUE;
    ...
    for (i = 3; i >= 0; i--) 
    {
        entryAddress[i] = (index << 2) | (uint8)i;
#ifdef RTL8306_LUT_CACHE
        ...
        {
            rtl8306e_l2_multicastEntry_set(macAddr, (uint8)i, isAuth, portMask);
 ...
 */ 
int8 rtl8306e_l2_mac_get(uint8 *macAddr, uint8 *pIsStatic, uint8 *pIsAuth, uint8 *pPortInfo, uint8 *pEntryaddr)
{
    int8  i;
    uint8 macAddress[6];
    uint16 index;
    uint8 entryAddress, age;
    uint8 isStatic, isAuth;
    uint8 port, portmask;
    uint8 isHit ;
 ...
 */ 
int8 rtl8306e_l2_mac_del(uint8 *macAddr, uint8 *pEntryaddr)
{
    uint8  i, port, portmask;
    uint8 macAddr1[6];
    uint16 index, entryaddr1;
    uint8 isStatic, isAuth, age;
    uint8 isHit = FALSE;
    uint8 hashStatus;
        ...
        for (i = 4; i >= 1; i --)
        {
            ...
            {  
               /*
                ...
                if (rtl8306e_l2_unicastEntry_set(macAddr1, (uint8)(i -1), 0, FALSE, FALSE, 0) != SUCCESS)
                    return FAILED;
                *pEntryaddr = entryaddr1;
                isHit = TRUE;
            } 
 ...
 * Note:
 */ 
int8 rtl8306e_l2_portMacLimit_set(uint8 port, uint8 enabled, uint8 macCnt)
{
    uint16 regValue;
 ...
 * Note:
 */ 
int8 rtl8306e_l2_portMacLimit_get(uint8 port, uint8 *pEnabled, uint8 *pMacCnt)
{
    uint16 regValue;
    uint8 bitValue;
 ...
 */ 
int8 rtl8306e_l2_systemMacLimit_set(uint8 enabled, uint8 macCnt, uint8 mergMask)
{
    uint16 regValue;
 ...
 * Note:
 */ 
int8 rtl8306e_l2_systemMacLimit_get(uint8 *pEnabled, uint8 *pMacCnt, uint8 *pMergMask)
{
    uint16 regValue;
    uint8 bitValue;
 ...
 * Note:
 */ 
int8 rtl8306e_l2_macLimitAction_set(uint8 action)
{
 ...
 * Note:
 */ 
int8 rtl8306e_l2_macLimitAction_get(uint8 *pAction)
{
    uint8 bitValue;
 ...
 */ 
int8 rtl8306e_stp_set(uint8 port, uint8 state)
{
    uint16 regValue;
 ...
 *         RTL8306_SPAN_FORWARD - Forwarding state
 */ 
int8 rtl8306e_stp_get(uint8 port, uint8 *pState)
{
    uint16 regValue;
 ...
 */ 
int8 rtl8306e_dot1x_portBased_set(uint8 port, uint8 enabled, uint8 isAuth, uint8 direction)
{
...
int8 rtl8306e_dot1x_portBased_get(uint8 port, uint8 *pEnabled, uint8 *pIsAuth, uint8 *pDirection)
{
    uint8 bitValue;
 ...
 */ 
int8 rtl8306e_dot1x_macBased_set(uint8 port, uint8 enabled, uint8 direction)
{
...
int8 rtl8306e_dot1x_macBased_get(uint8 port, uint8 *pEnabled, uint8 *pDirection)
{
    uint8 bitValue;
...
int8 rtl8306e_trap_igmpCtrlPktAction_set(uint8 type, uint8 action)
{    
    switch(type)
    {
        case RTL8306_IGMP:
...
int8 rtl8306e_trap_igmpCtrlPktAction_get(uint8 type, uint8 *pAction)
{
    uint8 regval;
 ...
 *          RTL8306_ACT_PERMIT   - normal forward
 */  
int8 rtl8306e_trap_unknownIPMcastPktAction_set(uint8 type, uint8 action)
{    
    switch(type)
    {
...
int8 rtl8306e_trap_unknownIPMcastPktAction_get(uint8 type, uint8 *pAction)
{
    uint8 regval;
...
int8 rtl8306e_trap_abnormalPktAction_set(uint8 type,  uint8 action)
{
    switch(type)
    {
...
int8 rtl8306e_trap_abnormalPktAction_get(uint8 type,  uint8 *pAction)
{
    uint8 regval;
...
int8 rtl8306e_trap_rmaAction_set(uint8 type, uint8 action)
{
...
int8 rtl8306e_trap_rmaAction_get(uint8 type, uint8 *pAction)
{
    uint8 bitValue1, bitValue2;
 ...
 *
 */ 
int8 rtl8306e_int_control_set(uint8 enInt, uint16 intmask)
{
    uint16 regValue;
 ...
 *
 */ 
int8 rtl8306e_int_control_get(uint8 *pEnInt, uint16 *pIntmask)
{
    uint16 regValue;
    uint8 bitValue;
 ...
 *
 */ 
int8 rtl8306e_int_status_get(uint16 *pStatusMask)
{
    uint16 regValue;
 ...
 */ 
int8 rtl8306e_storm_filterEnable_set(uint8 type, uint8 enabled)
{
...
int8 rtl8306e_storm_filterEnable_get(uint8 type, uint8 *pEnabled)
{
    uint8 bitValue;
...
int8 rtl8306e_storm_filter_set(uint8 trigNum, uint8 filTime, uint8 enStmInt)
{
    uint16 regval;
...
int8 rtl8306e_storm_filter_get(uint8 *pTrigNum, uint8 *pFilTime, uint8 *pEnStmInt)
{
    uint8 bitval;
    uint16 regval;
