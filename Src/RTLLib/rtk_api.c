...
{
    if( rtl8306e_asic_checkOUI() != SUCCESS ) {
...
rtk_api_ret_t rtk_port_macForceLinkExt0_get(rtk_mode_ext_t *pMode, rtk_port_mac_ability_t *pPortability)
{
    ...
    rtl8306e_regbit_get (0, 16, 14, 0, &bitVal);
    if (bitVal && (MODE_EXT_MII_PHY == *pMode))
        *pMode = MODE_EXT_TMII_PHY;
    
    if (bitVal && (MODE_EXT_MII_MAC == *pMode))
        *pMode = MODE_EXT_TMII_MAC;

    pPortability->forcemode = 1;
    rtl8306e_regbit_get (6, 22, 6, 0, &bitVal);
    pPortability->nway = bitVal ? 1 : 0;
    
    rtl8306e_regbit_get (6, 24, 13, 0, &bitVal);
    pPortability->rxpause = bitVal ? 0 : 1;

    rtl8306e_regbit_get (6, 24, 14, 0, &bitVal);
    pPortability->txpause = bitVal ? 0 : 1;

    rtl8306e_regbit_get (6, 22, 15, 0, &bitVal);
    pPortability->link= bitVal ? 1 : 0;
...
rtk_api_ret_t rtk_qos_1pRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if(NULL == pEnable)
        return RT_ERR_NULL_POINTER;
    if((retVal = rtl8306e_qos_1pRemarkEnable_get(port, (uint8 *)pEnable)) != SUCCESS)
        return RT_ERR_FAILED;    
    return RT_ERR_OK;
}
...
{
    uint16 vid_val,mbrmsk_val, untagmsk_val;
    uint8 index, hit_index, hit_flag;
    uint8 fullflag;
    int8 i;
    ...
    for(i = 15; i >= 0; i--)
    {
        rtl8306e_vlan_entry_get((uint8)i, &vid_val, &mbrmsk_val, &untagmsk_val);
        if (0 == vid_val) 
        {
            index = (uint8)i;
            fullflag = FALSE;
...
rtk_api_ret_t rtk_vlan_get(rtk_vlan_t vid, rtk_portmask_t *pMbrmsk, rtk_portmask_t *pUntagmsk, rtk_fid_t *pFid)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        rtl8306e_vlan_entry_get(i, &vid_val, &mbrmsk_val, &untagmsk_val);
        if(vid_val == vid)
        {
            hit_flag = TRUE;
...
rtk_api_ret_t rtk_vlan_destroy(rtk_vlan_t vid)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        if(vid_val == vid)
        {
            ...
            mbrmsk_val = 0;
            untagmsk_val = 0;
            rtl8306e_vlan_entry_set((uint8)i, vid_val, mbrmsk_val, untagmsk_val);
            return RT_ERR_OK;
        }        
    }
...
rtk_api_ret_t rtk_vlan_portPvid_set(rtk_port_t port, rtk_vlan_t pvid, rtk_pri_t priority)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        {
            hit_flag = TRUE;
            rtl8306e_vlan_portPvidIndex_set(port, (uint8)i);
...
rtk_api_ret_t rtk_vlan_vlanBasedPriority_set(rtk_vlan_t vid, rtk_pri_t priority)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        {
            hit_flag = TRUE;
            rtl8306e_vlan_vlanBasedPriority_set((uint8)i, priority);
            return RT_ERR_OK;
        }
    }
...
rtk_api_ret_t rtk_vlan_vlanBasedPriority_get(rtk_vlan_t vid, rtk_pri_t *pPriority)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        {
            hit_flag = TRUE;
            rtl8306e_vlan_vlanBasedPriority_get((uint8)i, pPriority);
            return RT_ERR_OK;
        }
    }
...
rtk_api_ret_t rtk_vlan_vidTranslation_set(rtk_vlan_t vid, rtk_vlan_t nvid)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        {
            hit_flag = TRUE;
            rtl8306e_vlan_transVid_set((uint8)i, nvid);
            return RT_ERR_OK;
        }        
    }
...
rtk_api_ret_t rtk_vlan_vidTranslation_get(rtk_vlan_t vid, rtk_vlan_t *pNvid)
{
    ...
    for (i = 15; i >= 0; i--)
    {
        ...
        {
            hit_flag = TRUE;
            rtl8306e_vlan_transVid_get((uint8)i, pNvid);
            return RT_ERR_OK;
        }        
    }
...
rtk_api_ret_t rtk_vlan_vidTranslationEnable_set(rtk_enable_t enable, rtk_portmask_t nniMask)
{
    ...
    retVal = rtl8306e_vlan_transEnable_set((uint8) enable, nniMask.bits[0]);
...
rtk_api_ret_t rtk_l2_limitLearningCntAction_set(rtk_port_t port, rtk_l2_limitLearnCntAction_t action)
{
    ...
    if ( LIMIT_LEARN_CNT_ACTION_DROP == action )
        datav = RTL8306_ACT_DROP;
    else if ( LIMIT_LEARN_CNT_ACTION_TO_CPU == action )
        datav = RTL8306_ACT_TRAPCPU;
    else
        return RT_ERR_NOT_ALLOWED;    

    
    if ((retVal = rtl8306e_l2_macLimitAction_set(datav)) != RT_ERR_OK)
        return retVal; 
...
{
    rtk_api_ret_t retVal;
    uint8 isAuth, direction, enDot1x;
    ...
    retVal = rtl8306e_dot1x_portBased_set(port, (uint8)enable, isAuth, direction);
    if(retVal != SUCCESS)
        return RT_ERR_FAILED;
...
rtk_api_ret_t rtk_dot1x_portBasedAuthStatus_set(rtk_port_t port, rtk_dot1x_auth_status_t port_auth)
{
    ...
    retVal = rtl8306e_dot1x_portBased_set(port, enDot1x, (uint8)port_auth, direction);
    if(retVal != SUCCESS)
        return RT_ERR_FAILED;
...
rtk_api_ret_t rtk_dot1x_portBasedDirection_set(rtk_port_t port, rtk_dot1x_direction_t port_direction)
{
    ...
    retVal = rtl8306e_dot1x_portBased_set(port, enDot1x, isAuth, (uint8)port_direction);
    if(retVal != SUCCESS)
        return RT_ERR_FAILED;
...
rtk_api_ret_t rtk_dot1x_macBasedEnable_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    retVal = rtl8306e_dot1x_macBased_set(port, (uint8)enable, direction);
    if (retVal != SUCCESS)
        return RT_ERR_FAILED;
...
rtk_api_ret_t rtk_dot1x_macBasedAuthMac_add(rtk_port_t port, rtk_mac_t *pAuth_mac, rtk_fid_t fid)
{
    ...
    uint8 macAddr[6];
    uint8 entryaddr;
    uint8 isStatic,isAuth,age, srcPort;
    uint8 isHit;
    uint16 index;
    ...
    for (i = 3; i >= 0; i--) 
    {
                   ...
                   (pAuth_mac->octet[4] == macAddr[4]) && (pAuth_mac->octet[5] == macAddr[5])) 
        {
            if(srcPort != port)
                return RT_ERR_DOT1X_MAC_PORT_MISMATCH;
            if (rtl8306e_l2_unicastEntry_set(pAuth_mac->octet, (uint8)i , age, isStatic, TRUE, port) != SUCCESS)
                return RT_ERR_FAILED;
            isHit = TRUE;
            return RT_ERR_OK;
        }
...
rtk_api_ret_t rtk_dot1x_macBasedAuthMac_del(rtk_port_t port, rtk_mac_t *pAuth_mac, rtk_fid_t fid)
{
    ...
    for (i = 3; i >= 0; i--) 
    {
                   ...
                   (pAuth_mac->octet[4] == macAddr[4]) && (pAuth_mac->octet[5] == macAddr[5])) 
        {
            if(srcPort != port)
                return RT_ERR_DOT1X_MAC_PORT_MISMATCH;
            if (rtl8306e_l2_unicastEntry_set(pAuth_mac->octet, (uint8)i , age, isStatic, FALSE, port) != SUCCESS)
                return RT_ERR_FAILED;
            isHit = TRUE;
            return RT_ERR_OK;
        }
...
rtk_api_ret_t rtk_svlan_tpidEntry_set(rtk_svlan_tpid_t svlan_tag_id)
{
    ...
    rtl8306e_reg_set(6, 28, 3, (uint16)svlan_tag_id);
...
{
    uint8 entaddr,port, pro, pri, act;
    uint16 val;
    uint8 isFull ;
    ...
    for (entaddr = 0; entaddr < RTL8306_ACL_ENTRYNUM; entaddr ++ ) 
    {
        ...
        if ((pRule->phyport == port) && (pRule->protocol == pro) && (pRule->datavalue == val))
        {
            if (rtl8306e_acl_entry_set(entaddr, pRule->phyport, pRule->action, pRule->protocol, pRule->datavalue, pRule->priority) != SUCCESS )
                 return RT_ERR_FAILED;
            else
                 return RT_ERR_OK;
        }            
...
rtk_api_ret_t rtk_filter_igrAcl_rule_del(rtk_filter_rule_t *pRule)
{
    ...
    for (entaddr = 0; entaddr < RTL8306_ACL_ENTRYNUM; entaddr ++ ) 
    {
        ...
        if ((port == pRule->phyport) && (pro == pRule->protocol) && (val == pRule->datavalue))
        {
